...
// Pointer types
LRESULT validLRESULT = 1234;  // LRESULT is typically a type for return values in Windows messages
...
PDWORDLONG validPDWORDLONG = new DWORDLONG(9876543210);  // PDWORDLONG is a pointer to a DWORDLONG (64-bit unsigned)
PDWORD_PTR validPDWORD_PTR = new DWORD_PTR(456);  // PDWORD_PTR is a pointer to DWORD_PTR (pointer-sized unsigned integer)
PDWORD32 validPDWORD32 = new DWORD32(1234);  // PDWORD32 is a pointer to DWORD32 (32-bit unsigned)
PDWORD64 validPDWORD64 = new DWORD64(1234567890123456);  // PDWORD64 is a pointer to DWORD64 (64-bit unsigned)
...
PHANDLE validPHANDLE = new HANDLE(reinterpret_cast<HANDLE>(1234));  // PHANDLE is a pointer to a HANDLE
PHKEY validPHKEY = new HKEY(reinterpret_cast<HKEY>(5678));  // PHKEY is a pointer to a HKEY (registry handle)
PINT validPINT = new INT(42);  // PINT is a pointer to an INT (signed integer)
PINT_PTR validPINT_PTR = new INT_PTR(789);  // PINT_PTR is a pointer to an INT_PTR (pointer-sized signed integer)
PINT32 validPINT32 = new INT32(234);  // PINT32 is a pointer to an INT32 (32-bit signed integer)
PINT64 validPINT64 = new INT64(56789012345);  // PINT64 is a pointer to an INT64 (64-bit signed integer)
...
PLONGLONG validPLONGLONG = new LONGLONG(10000000000);  // PLONGLONG is a pointer to a LONGLONG (signed 64-bit)
PLONG_PTR validPLONG_PTR = new LONG_PTR(500);  // PLONG_PTR is a pointer to LONG_PTR (pointer-sized signed integer)
PLONG32 validPLONG32 = new LONG32(999);  // PLONG32 is a pointer to LONG32 (32-bit signed integer)
PLONG64 validPLONG64 = new LONG64(456789012345);  // PLONG64 is a pointer to LONG64 (64-bit signed integer)
PSHORT validPSHORT = new SHORT(300);  // PSHORT is a pointer to a SHORT (signed 16-bit)
PSIZE_T validPSIZE_T = new SIZE_T(10000);  // PSIZE_T is a pointer to SIZE_T (platform-dependent size type)
PSSIZE_T validPSSIZE_T = new SSIZE_T(-123);  // PSSIZE_T is a pointer to SSIZE_T (signed platform-dependent size type)
PSTR validPSTR = new CHAR[6]{'H', 'e', 'l', 'l', 'o', '\0'};  // PSTR is a pointer to an ANSI string
BYTE* validPTBYTE = new BYTE[5]{0x01, 0x02, 0x03, 0x04, 0x05};  // PTBYTE is a pointer to an array of bytes
CHAR* validPTCHAR = new CHAR[3]{'A', 'B', 'C'};  // PTCHAR is a pointer to an array of chars (ANSI or Unicode)
CHAR* validPTSTR = new CHAR[5]{'T', 'e', 's', 't', '\0'};  // PTSTR is a pointer to an ANSI or Unicode string
PUCHAR validPUCHAR = new UCHAR(0xA);  // PUCHAR is a pointer to an unsigned char
PUHALF_PTR validPUHALF_PTR = nullptr;  // Pointers to half-precision floating-point (use if needed)
PUINT validPUINT = new UINT(256);  // PUINT is a pointer to a UINT (unsigned 32-bit)
PUINT_PTR validPUINT_PTR = new UINT_PTR(123456);  // PUINT_PTR is a pointer to UINT_PTR (pointer-sized unsigned integer)
PUINT32 validPUINT32 = new UINT32(256);  // PUINT32 is a pointer to UINT32 (32-bit unsigned integer)
PUINT64 validPUINT64 = new UINT64(9876543210);  // PUINT64 is a pointer to UINT64 (64-bit unsigned integer)
PULONG validPULONG = new ULONG(789);  // PULONG is a pointer to ULONG (32-bit unsigned)
PULONGLONG validPULONGLONG = new ULONGLONG(12345678901234);  // PULONGLONG is a pointer to ULONGLONG (64-bit unsigned)
PULONG_PTR validPULONG_PTR = new ULONG_PTR(999);  // PULONG_PTR is a pointer to ULONG_PTR (pointer-sized unsigned integer)
PULONG32 validPULONG32 = new ULONG32(456);  // PULONG32 is a pointer to ULONG32 (32-bit unsigned)
PULONG64 validPULONG64 = new ULONG64(654321987654321);  // PULONG64 is a pointer to ULONG64 (64-bit unsigned)
...
SC_HANDLE validSC_HANDLE = reinterpret_cast<SC_HANDLE>(0x12345);  // SC_HANDLE is a pointer to a service control manager handle
SC_LOCK validSC_LOCK = reinterpret_cast<SC_LOCK>(0x67890);  // SC_LOCK is a pointer to a service control lock
