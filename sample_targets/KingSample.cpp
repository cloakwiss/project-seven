...
void SafeCreateProcess(const char* cmdLine) {
    STARTUPINFOA si = { sizeof(si) };
    PROCESS_INFORMATION pi = { 0 };

    // Create the process and check if it succeeded
    if (!CreateProcessA(
        NULL,           // Application name (NULL means use cmdLine)
...
void SafeSendMessage(HWND hwnd) {
...
void SafeMemoryOperations() {
    ...
    if (VirtualProtect(buffer, memSize, oldProtect, &oldProtect) == 0) {
        std::cerr << "Restoring original memory protection failed. Error: " << GetLastError() << std::endl;
    } else {
...
void SafeWriteProcessMemory() {
    const char* data = "World";
    SIZE_T bytesWritten;

    // Write memory in the current process
    if (!WriteProcessMemory(GetCurrentProcess(), (LPVOID)0x12345678, (LPCVOID)data, 6, &bytesWritten)) {
